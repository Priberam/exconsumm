...
bool NoNameYetModel::Train(Corpus& train_corpus, Corpus& dev_corpus, const std::string& output_path, const std::string& evaluate_command)
{
    ...
    std::cerr << "NUMBER OF TRAINING DOCUMENTS: " << si << std::endl;
    unsigned trs = 0;
    double right = 0;
    double llh = 0;
    bool first = true;
    ...
    while (!requested_stop) {
        ...
        std::vector<dynet::Expression> losses;
        {
            ...
            for (unsigned sii = 0; sii < status_every_i_iterations; ++sii, si++) {
                ...
                if (si % 1000 == 0 && m_config->ENABLE_REINFORCEMENT_LEARNING) {
                    if (iepoch >= 1) {
                        ...
                        m_config->Logger("asum") << "Seen all corpus once | " << "num_RL_tries: " << num_RL_tries << std::endl;
                        ...
                        m_config->Logger("asum") << "Seen " << corpus_perc << " % documents | " << "num_RL_tries: " << num_RL_tries << std::endl;
            ...
            std::cerr << one_pred << " " << one_total << " " << one_ok << " " << f1;
            f1 += 0.0000001;
        ...
        if ((logc % 1000 == 1 || eval) && logc > last +2) { // report on dev set
            ...
            m_config->Logger("asum") << "[(extr scores:)] **P = " << one_ok / (double)one_pred << " r = " << one_ok / (double)one_total << " f1= " << f1 << " n = " << one_pred / (double)dev_size << " MAX_RELATION = " << m_config->MAX_RELATION << std::endl;
            m_config->Logger("asum") << "[(extr scores:)] ** best P = " << best_f1_p << " best r = " << best_f1_r << " best f1 = " << best_f1 << " best epoch = " << best_f1_epoch << std::endl;
            command = evaluate_command + " --path " + output_path + "/dev/  --type asum " +
                "--model " + fname_parameters + "_e_" + std::to_string(nepochs) + " --out " + m_config->LoggerFilePath("asum") + " &";
            m_config->Logger("asum") << command << std::endl;
            system(command.c_str());
            m_config->Logger("asum") << std::endl;

            auto asumlog = m_config->LoggerFilePath("asum");
            std::ifstream asumfile(asumlog);
            asumfile.seekg(-1, std::ios::end);
            auto pos = asumfile.tellg();
            ...
            if (pos > 1000) {
                asumfile.seekg(-1000, std::ios::end);
                std::string line;
                
                while (std::getline(asumfile, line)) {
                    if (line.find("ROUGE Evaluation ") != std::string::npos) {
                        std::vector<std::string> toks;
                        string_split(line, &toks, " ");
                        ...
                        // dont break; if it finds another use the last rouge reported as needed
                    }
                }
            }
            ...
            std::cout << "Abs R1_F/R2_F: ("<< R1_F <<  " , " << R2_F << ")" << std::endl;
            m_config->Logger("asum") << "Abs R1_F/R2_F: (" << R1_F << " , " << R2_F << ")" << std::endl;
            float abs_f1 = (R1_F + R2_F) / 2.0f;
            std::cout << "Abs 'f1': " << abs_f1 << std::endl;
            m_config->Logger("asum") << "Abs 'f1': " << abs_f1 << std::endl;
            ...
            if (best_f1 < f1 && nepochs > 0) {
                ...
                m_config->Logger("asum") << "saving new BEST-f1 to model_fname: " << model_fname << std::endl;
